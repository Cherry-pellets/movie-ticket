...
<script>
    import {getCurrentPageBMovieSchedule,getOptions2,getHallByCinemaId,addScheduleInfo,deleteMovieSchedule} from '@/api/movie'
    ...
    export default {
        ...
        methods: {
          async loadCurrentPageMovieSchedule(currentPage,pageSize,input){
            ...
            let {data, status} = await getCurrentPageBMovieSchedule({
              pageNum: currentPage,
              limit: pageSize,
              keyword: input,
              cinemaId: cinemaId
          ...
          async handleDelete(index, row) {
            MessageBox.confirm('此操作将永久删除该电影排片所有信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
                let {data, status} = await deleteMovieSchedule({
                  timesId: row.id,
                  daysId:row.days.id
                });
                if (status === 200 && data.state===200){
          ...
          async addSchedule(){
            this.scheduleInfo = {};
            this.scheduleInfo.cinemaId = localStorage.getItem("cinemaId");
            this.moviePublicDate = '';
            let {data, status} = await getOptions2({});
            if (status === 200 && data.state===200){
              this.movieOptions = data.data.movies;
            }
            this.getHall();
          ...
          async getHall(){
            this.hallOptions = [];
            let {data, status} = await getHallByCinemaId({
              cinemaId: localStorage.getItem("cinemaId")
            });
            if (status === 200 && data.state===200){
              this.hallOptions = data.data;
            }
          },
          //选择电影
          changeMovie(value){
          ...
          async addScheduleInfo(){
            ...
            if (this.scheduleInfo.movieId&&this.scheduleInfo.cinemaId&&this.scheduleInfo.hallId
              ...
              let {data, status} = await addScheduleInfo({
                movieId: this.scheduleInfo.movieId,
                cinemaId: this.scheduleInfo.cinemaId,
                hallId: this.scheduleInfo.hallId,
                showDate: this.scheduleInfo.movieShowDate,
                showTime: this.scheduleInfo.movieShowTime,
