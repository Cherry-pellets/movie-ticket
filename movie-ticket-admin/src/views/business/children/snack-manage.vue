...
<script>
    import {getBSnacks,updateSnackInfo,deleteSnackInfo} from '@/api/order'
        ...
        created(){
         ...
         methods: {
          async loadCurrentPageSnack(pageNum,limit,input){
            ...
            let {data, status} = await getBSnacks({
              pageNum: pageNum,
              limit: limit,
              keyword: input,
              cinemaId: cinemaId
          ...
          handleEdit(index, row) {
            ...
            this.snackInfo = row;
            this.snackInfo.cinemaId = localStorage.getItem("cinemaId");
          ...
          async handleDelete(index, row) {
            MessageBox.confirm('此操作将永久删除该小吃信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
                let {data, status} = await deleteSnackInfo({snackId: row.id});
                if (status === 200 && data.state===200){
                  this.loadCurrentPageSnack(this.currentPage,8,this.searchInput);
            ...
            console.log("test")
             if (this.snackInfo.cinemaId&&this.snackInfo.imageUrl&&this.snackInfo.firstTitle&&this.snackInfo.secondTitle&&this.snackInfo.price&&this.snackInfo.totalNumber) {
                ...
                if (this.dialogTitle === '编辑小吃信息') {
                  let {data, status} = await updateSnackInfo({snack: this.snackInfo});
                  if (status === 200 && data.state===200){
