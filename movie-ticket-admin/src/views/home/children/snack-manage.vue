...
<script>
    import {getSnacks,updateSnackInfo,deleteSnackInfo,getOptions} from '@/api/order'
    ...
    export default {
        ...
        methods: {
          async loadCurrentPageSnack(pageNum,limit,input){
            let {data, status} = await getSnacks({
              pageNum: pageNum,
              limit: limit,
              keyword: input
            });
          ...
          async handleDelete(index, row) {
            MessageBox.confirm('此操作将永久删除该小吃信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
                let {data, status} = await deleteSnackInfo({
                  snackId: row.id
                });
                if (status === 200 && data.state===200){
                  this.loadCurrentPageSnack(this.currentPage,8,this.searchInput);
          ...
          async manageSnackInfo(){
            if (this.snackInfo.imageUrl&&this.snackInfo.firstTitle&&this.snackInfo.secondTitle&&this.snackInfo.price&&this.snackInfo.totalNumber) {
                ...
                if (this.dialogTitle === '编辑小吃信息') {
                  let {data, status} = await updateSnackInfo({
