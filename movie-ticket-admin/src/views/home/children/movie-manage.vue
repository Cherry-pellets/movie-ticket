              ...
              <el-row :span="16">
                <ul class="upload-imgs">
                  <li v-for='(value,index) in newPhotos' :key="index">
                    <p  v-if="isNew" class="img"><img :src="value" width="150px" height="90px"><a class="close" @click="delImg2(index)">×</a></p>
                  </li>
                  <li v-for='(value,index) in movieInfo.photosList' :key="index">
                    <p  v-if="!isNew" class="img"><img :src="value" width="150px" height="90px"><a class="close" @click="delImg1(index)">×</a></p>
                  </li>
                </ul>
                <div style="position: relative;height: 20px">
                  <el-button size="small" type="primary" style="position: absolute;left: 0;top: 0;cursor: pointer">点击上传</el-button>
    ...
    export default {
        ...
        methods: {
          async loadCurrentPageMovie(pageNum,limit,input){
            let {data, status} = await getCurrentPageMovie({
              pageNum: pageNum,
              limit: limit,
              input: input
            });
          ...
          async handleDelete(index, row) {
            ...
            MessageBox.confirm('此操作将永久删除该电影所有信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
                let {data, status} = await deleteMovieInfo({
                  movieId: row.id
                });
                if (status === 200 && data.state===200){
                  this.loadCurrentPageMovie(this.currentPage,8,this.searchInput);
          ...
          async manageMovieInfo(){
            if (this.movieInfo.nm&&this.movieInfo.img&&this.movieInfo.dir&&this.movieInfo.star&&this.movieInfo.dur&&this.movieInfo.cat&&this.movieInfo.language&&this.movieInfo.rt&&this.movieInfo.dra) {
              ...
              if (status === 200 && data.state===200){
                ...
                //判断是编辑还是添加
                if (this.dialogTitle === '编辑电影信息') {
                  let {data, status} = await updateMovieInfo({
