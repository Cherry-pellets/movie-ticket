...
<script>
    ...
    import {getCurrentPageCinema,updateCinemaInfo,deleteCinemaInfo,getBrandList,addCinemaBrand} from '@/api/cinema'
    import Vue from "vue"
    export default {
        name: "CinemaManage",
        data() {
        ...
        methods: {
          async loadCurrentPageCinema(currentPage,pageSize,input){
            let {data, status} = await getCurrentPageCinema({
              pageNum: currentPage,
              limit: pageSize,
              keyword:input
            });
          ...
          async loadBrandList(){
            let {data, status} = await getBrandList({});
            if (status === 200 && data.state===200){
          ...
          handleDelete(index, row) {
            ...
            MessageBox.confirm('此操作将永久删除该影院所有信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
                let {data, status} = await deleteCinemaInfo({
                  cinemaId: row.id
                  });
                if (status === 200 && data.state===200){
          ...
          async manageCinemaInfo(){
            if (!this.cinemaInfo.nm||!this.cinemaInfo.addr) {
              ...
              if(reg.test(this.newBrand)){
                let {data, status} = await addCinemaBrand({
                  newBrand: this.newBrand
                  });
                if (status === 200 && data.state===200){
                  this.cinemaInfo.brandId = data.data;
                  this.newBrand = '';
                  this.loadBrandList();
                } else{
              ...
              if (this.dialogTitle==='编辑影院信息') {
                  let {data, status} = await updateCinemaInfo({
